...
void Button2Chord::doubleChord(const set<VirtualChordButton> &set_of_chord) {
  ...
  if (vcb2.cbt == ChordButtonType::dim) {
    // 1個目がメジャーでないなら違う
    if (vcb1.cbt != ChordButtonType::Major)
      return;

    // 11個ずれならセブンス
    if ((static_cast<int>(vcb1.note) + DIM_DIFF + 11) % NUM_OF_NOTE ==
        static_cast<int>(vcb2.note)) {
      m_chord_name.root_note = vcb1.note;
      m_chord_name.chord_type = sev;
    }
    // メジャーとディミニッシュの列が同じならsus4
    else if ((static_cast<int>(vcb1.note) + DIM_DIFF) % NUM_OF_NOTE ==
             static_cast<int>(vcb2.note)) {
      m_chord_name.root_note = vcb1.note;
  ...
  if (vcb2.cbt == ChordButtonType::minor) {
    ...
    // ずれがないならマイナーセブンス
    if ((static_cast<int>(vcb1.note) + MIN_DIFF) % NUM_OF_NOTE ==
        static_cast<int>(vcb2.note)) {
    ...
    // 1つズレならメジャーセブンス
    else if ((static_cast<int>(vcb1.note) + MIN_DIFF + 1) % NUM_OF_NOTE ==
             static_cast<int>(vcb2.note)) {
      m_chord_name.root_note = vcb1.note;
      m_chord_name.chord_type = Msev;
      return;
  ...
  if (vcb2.cbt == ChordButtonType::sus4) {
    if (vcb1.cbt == ChordButtonType::dim) {
      if ((static_cast<int>(vcb2.note) + DIM_DIFF + 11) % NUM_OF_NOTE !=
          static_cast<int>(vcb1.note))
        return;
      m_chord_name.root_note = vcb2.note;
      m_chord_name.chord_type = sevsus4;
    } else if (vcb1.cbt == ChordButtonType::minor) {
      if ((static_cast<int>(vcb2.note) + MIN_DIFF + 1) % NUM_OF_NOTE !=
          static_cast<int>(vcb1.note))
        m_chord_name.root_note = vcb2.note;
  ...
  if (vcb2.cbt == ChordButtonType::arg) {
    ...
    if (vcb1.cbt == ChordButtonType::minor) {
      // ずれなしでないなら違う
      if (static_cast<int>(vcb1.note) !=
          (static_cast<int>(vcb2.note) + MIN_DIFF) % 12)
        return;
      m_chord_name.root_note = vcb1.note;
  ...
  if (vcb2.cbt == ChordButtonType::Major) {
    ...
    // 1音ずれでないなら違う
    if ((static_cast<int>(vcb1.note) + 1) % 12 !=
        (static_cast<int>(vcb2.note)) % 12)
      return;
    m_chord_name.root_note = vcb1.note;
    m_chord_name.chord_type = add9;
  }
...
void Button2Chord::notifyPressedButton(
    const std::set<VirtualChordButton> &input) {
...
/// @brief
/// 押されているコードボタンからコードネームを決定し、配下の関数を使ってchord_nameに格納する。
...
void Button2Chord::addPressedButtonObserver(
    function<void(const set<VirtualChordButton> &)> func) {
  m_pressed_button_observer.push_back(func);
}

void Button2Chord::addCurrentChordObserver(
    std::function<void(ChordName)> func) {
